...
export class JobAppComponent implements OnInit {
  ...
  trackId(index: number, item: IJobApp): number {
    return item.id!;
  }

  delete(job: IJobApp): void {
    const modalRef = this.modalService.open(JobAppDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IJobApp[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
